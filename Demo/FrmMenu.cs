    ...
    public partial class FrmMenu : Form
    {
        ...
        private void Draw_Graphics(DataTable boxDataTable, int layer)
        {
            ...
            foreach (DataRow boxRow in boxDataTable.Rows)
            {
                ...
                element.Width = int.Parse(boxRow["P2X"].ToString()) - int.Parse(boxRow["P1X"].ToString());
                element.Height = int.Parse(boxRow["P3Y"].ToString()) - int.Parse(boxRow["P2Y"].ToString());

                // set element lower left corner coordinates
                element.X = int.Parse(boxRow["P4X"].ToString());
                element.Y = PalletHeight - int.Parse(boxRow["P4Y"].ToString());
                ...
                // draw index value
                _drawingTable.DrawString(boxRow["Box ID"].ToString(), font, brush, indexCoordinates);
                _drawingTable.DrawString(boxRow["Height"].ToString(), font, brush, heightCoordinates, format);
            }
